...
export async function POST(req: NextRequest) {
  ...
  try {
    const { songId } = UpvoteSchema.parse(await req.json());
    ...
    await prisma.songUpVote.create({
      data: {
        userId: user.id,
        songId,
      },
    ...
    // 1️⃣ Increment song score in stream queue
    await redis.zincrby(`queue:stream:${song.streamId}`, 1, songId);

    // 2️⃣ Track user vote
    await redis.sadd(`song:${songId}:upvotes`, user.id);
